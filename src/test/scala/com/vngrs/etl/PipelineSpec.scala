...
class PipelineSpec extends BaseSparkSpec {
  ...
  it should "transform data" in {
    ...
    val transformedPipeline = pipeline.transform(Transform(i => Math.pow(i, 2).toInt))
  ...
  it should "accept a functionless combiner and result should be itself" in {
    ...
    val combinedPipeline = pipeline.combine(new Combine[Int, Int]{
      override def apply(pipeline2: Pipeline[Int]): Pipeline[Int] = pipeline2
    })
