...
/**
  ...
  */
class CombineSpec extends SparkSpec {
  ...
  "A functionless Combiner" should "return given Pipeline with an Empty RDD as it is" in {
    ...
    val combiner = new Combine[String, String, String](pipeline) {
      override def apply(pipeline2: Pipeline[String]): Pipeline[String] = pipeline
    }
  ...
  it should "return given Pipeline with any non empty RDD without changing data in it" in {
    ...
    combinedPipeline.testCollect should contain theSameElementsAs data
  }
